...
def urn_from_text(T):
    ...
    Args:
        T (str): Text containing URNs

    Returns:
        List[str]: All URNs found within the given text
    """
...
def get_papers(top=5, cutoff=5, navn='%', yearfrom=1800, yearto=2020, samplesize=100):
    ...
    Returns:
        List[Dict]: Newspapers from the API
    """
    def div(x, y): return (int(x / y), x % y)
    chunks = 20
...
def convert_list_of_freqs_to_dataframe(referanse):
    ...
    Args:
        referanse (List[Dict]): Newspapers from the API
    """
    res = []
    for x in referanse:
        res.append(dict(x))
...
def get_corpus(top=5, cutoff=5, navn='%', corpus='avis', yearfrom=1800, yearto=2020, samplesize=10):
    if corpus == 'avis':
        ...
        urns = get_urn({'author': navn, 'year': yearfrom,
                        'neste': yearto - yearfrom, 'limit': samplesize})
...
class Cluster:
        ...
        if word != '':
            ...
            if type(reference) is pd.core.frame.DataFrame:
                ...
                reference = get_corpus(
                    yearfrom=period[0], yearto=period[1], corpus=corpus, samplesize=reference)
            else:
                reference = get_corpus(
                    yearfrom=period[0], yearto=period[1], corpus=corpus, samplesize=int(reference))
    ...
    def save(self, filename=''):
        if filename == '':
            filename = "{w}_{p}-{q}.json".format(
                w=self.word, p=self.period[0], q=self.period[1])
        model = {
            'word': self.word,
            'period': self.period,
    ...
    def search_words(self, words, exponent=1.1):
        ...
        res = df.transpose()[sub].transpose().sort_values(
...
def draw_graph_centrality(G, h=15, v=10, fontsize=20, k=0.2, arrows=False, font_color='black', threshold=0.01): 
    ...
    nx.draw_networkx_nodes(G, pos, alpha=0.5, nodelist=subnodes.keys(), node_size=[
                           v * 1000 for v in subnodes.values()])
...
def combine(clusters):
    ...
    for c in clusters[1:]:
        collocates = collocates.join(
            c.collocates, rsuffix='-' + str(c.period[0]))
    return collocates
...
def les_serie_cluster(word, startår, sluttår, inkrement):
    ...
    for i in range(startår, sluttår, inkrement):
        print(i, i + inkrement - 1)
        tcluster[i] = Cluster(
            filename='{w}_{f}-{t}.json'.format(w=word, f=i, t=i + inkrement - 1))
    return tcluster
...
def make_a_collocation(word, period=(1990, 2000), before=5, after=5, corpus='avis', samplesize=100, limit=2000):
    ...
    reference = get_corpus(
        yearfrom=period[0], yearto=period[1], samplesize=samplesize)
...
class Corpus:
    def __init__(self, filename = '', period = (1950,1960), author='%', 
        ...
        if filename == '':
            ...
            mål_docf = pd.DataFrame(pd.DataFrame(
                målkorpus_txt / målkorpus_txt).sum(axis=1))
            combo_docf = pd.DataFrame(pd.DataFrame(combo / combo).sum(axis=1))
            ref_docf = pd.DataFrame(pd.DataFrame(
                referanse_txt / referanse_txt).sum(axis=1))
    ...
    def search_words(self, words, freq_exp=1.1, doc_exp=1.1):
        if type(words) is str:
            words = [w.strip() for w in words.split()]
        df = self.difference(
            freq_exp=freq_exp, doc_exp=doc_exp, top=0, aslist=False)
        sub = [w for w in words if w in df.index]
        res = df.transpose()[sub].transpose().sort_values(
            by=df.columns[0], ascending=False)
        return res
...
def plot_sammen_vekst(urn, ordlister, window=5000, pr=100):
    ...
    for ordbag in ordlister:
        vekst = vekstdiagram(
            urn, params={'words': ordbag, 'window': window, 'pr': pr})
...
def ngram_conv(ngrams, smooth=1, years=(1810, 2013), mode='relative'):
    ...
    for x in ngrams:
        if x != []:
            ngc[x['key']] = {z['x']: z[arg] for z in x['values'] if int(
                z['x']) <= years[1] and int(z['x']) >= years[0]}
    return pd.DataFrame(ngc).rolling(window=smooth, win_type='triang').mean()


def make_graph(word):
    result = requests.get(
        "https://www.nb.no/sp_tjenester/beta/ngram_1/galaxies/query?terms={word}".format(word=word))
    G = nx.DiGraph()
    edgelist = []
    ...
    if kind == 'html':
        ...
        try:
            if corpus == 'bok':
                ...
                res = res[['urn', 'author', 'title',
                           'year', 'before', 'word', 'after']]
            else:
                res = pd.DataFrame(r.json())
...
def konk_to_html(jsonkonk):
    """Convert a collection of concordances to an HTML table.

    Args:
        jsonkonk (Iterable[Mapping[str, str]]): Collection of concordances.
            Concordance objects have the keys ['urn', 'before', 'word', 'after']

    Returns:
        str: HTML table of the concordances
    """
    rows = ""
    row_template = ("<tr>"
                    "<td><a href='{urn}' target='_'>{urnredux}</a></td>"
        ...
        for x in r.json():
            rows += """<tr>
                ...
                <td style='text-align:left'>{a}</td>
                              ...
                              urnredux="{t}, {f}, {y}".format(
                                  t=x['title'], f=x['author'], y=x['year']),
                              b=x['before'],
                              w=x['word'],
                              a=x['after']
                              )
